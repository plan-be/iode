...
Si c'est la cas,
le caractŠre de retour … la ligne est ins‚r‚ entre les lignes ~c(\n)~C.
...
… l'‚cran et une r‚ponse est attendue: si un des caractŠres "OoJjYy1" est entr‚, la r‚ponse
est positive et le rapport se poursuit au label indiqu‚.


&IT Noms des fichiers ascii aprŠs lecture dans IODE
...
Dor‚navant, lorsqu'un fichier ascii est lu dans IODE, par exemple ~c"test.av",~C ou
    "test.xxx" 2 cas sont distingu‚s :

&EN si le fichier existe, le nom du WS retient onserve le nom du fichier lu mais l'extension
    est remplac‚e par l'extension ascii standard (ici ~c"test.av"~C, mˆme pour ~c"test.xxx"~C );
