...
TEST_F(KDBIdentitiesTest, Subset)
{
    ...
    // DEEP COPY SUBSET
    std::shared_ptr<KDBIdentities> kdb_subset_deep_copy = new KDBIdentities(global_ws_idt, pattern, true);
    ...
    // SHALLOW COPY SUBSET
    std::shared_ptr<KDBIdentities> kdb_subset_shallow_copy = new KDBIdentities(global_ws_idt, pattern, false);
...
TEST_F(KDBIdentitiesTest, Filter)
{
    ...
    kdb_subset = new KDBIdentities(global_ws_idt, pattern, false);
    ...
    // wrong pattern
    pattern = "anjfks";
    EXPECT_THROW(KDBIdentities(global_ws_idt, pattern, false), std::runtime_error);
}
...
TEST_F(KDBIdentitiesTest, DeepCopy)
{
    ...
    kdb_subset = new KDBIdentities(global_ws_idt, pattern, true);
    EXPECT_EQ(kdb_subset->size(), expected_names.size());
    EXPECT_EQ(kdb_subset->get_names(), expected_names);
...
TEST_F(KDBIdentitiesTest, Merge)
{
    ...
    std::shared_ptr<KDBIdentities> kdb0 = new KDBIdentities(global_ws_idt, pattern, true);
    std::shared_ptr<KDBIdentities> kdb1 = new KDBIdentities(global_ws_idt, pattern, true);
    std::shared_ptr<KDBIdentities> kdb_to_merge = new KDBIdentities(global_ws_idt, pattern, true);
