...
TEST_F(SubsetsTest, AddDeletePtr)
{
    ...
    // check that the same pointer is used by scl_ptr and the global KDB (no copy)
    EXPECT_TRUE(scl_global_ws == scl_ptr);
    ...
    EXPECT_TRUE(scl_subset == scl_ptr);
    EXPECT_TRUE(scl_subset == scl_global_ws);
    // check that resetting the pointer scl_ptr does not reset the pointer in 
    // the global KDB and in the subset
    scl_ptr.reset();
    EXPECT_TRUE(scl_ptr.get() == nullptr);
