...
TEST_F(KDBListsTest, Subset)
{
    ...
    // DEEP COPY SUBSET
    std::shared_ptr<KDBLists> kdb_subset_deep_copy = new KDBLists(global_ws_lst, pattern, true);
    ...
    // SHALLOW COPY SUBSET
    std::shared_ptr<KDBLists> kdb_subset_shallow_copy = new KDBLists(global_ws_lst, pattern, false);
...
TEST_F(KDBListsTest, Filter)
{
    ...
    kdb_subset = new KDBLists(global_ws_lst, pattern, false);
    ...
    // wrong pattern
    pattern = "anjfks";
    EXPECT_THROW(KDBLists(global_ws_lst, pattern, false), std::runtime_error);
}
...
TEST_F(KDBListsTest, DeepCopy)
{
    ...
    kdb_subset = new KDBLists(global_ws_lst, pattern, true);
    EXPECT_EQ(kdb_subset->size(), expected_names.size());
    EXPECT_EQ(kdb_subset->get_names(), expected_names);
...
TEST_F(KDBListsTest, Merge)
{
    ...
    std::shared_ptr<KDBLists> kdb0 = new KDBLists(global_ws_lst, pattern, true);
    std::shared_ptr<KDBLists> kdb1 = new KDBLists(global_ws_lst, pattern, true);
    std::shared_ptr<KDBLists> kdb_to_merge = new KDBLists(global_ws_lst, pattern, true);
