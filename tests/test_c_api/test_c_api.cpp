...
public:
	...
	void U_test_CreateObjects()
	{
	    ...
	    // For B_DataPattern()
	    success = K_add(kdb_lst, "AB", "A,B");
        EXPECT_TRUE(success);
	    success = K_add(kdb_lst, "BC", "B,C");
        EXPECT_TRUE(success);
	    success = K_add(kdb_var, "AB", B, &nb);
        EXPECT_TRUE(success);
	    success = K_add(kdb_var, "AC", B, &nb);
	    EXPECT_TRUE(success);
        success = K_add(kdb_var, "BB", B, &nb);
        EXPECT_TRUE(success);
        success = K_add(kdb_var, "BC", B, &nb);
        EXPECT_TRUE(success);
	}
	...
	bool U_test_B_WsCopyVar()
	{
	    ...
	    success = K_add(KV_WS, "ACAF", ACAF, &nb);
        EXPECT_TRUE(success);
...
TEST_F(IodeCAPITest, Tests_B_DATA)
{
    ...
    // B_DataListSort()
    success = K_add(KL_WS, "LIST1", "A;C;B");
    EXPECT_TRUE(success);
...
TEST_F(IodeCAPITest, Tests_BIG_WS)
{
    ...
    {
        std::string command = "python " + str_tests_dir + "generate_big_vars_ws.py";
        std::cout << "Executing command: '" << command << "'" << std::endl;
        int result = system((char*) command.c_str());
        if(result == 0)
            std::cout << "Successfully generated big.var and big.av test files" << std::endl;
        else
            std::cout << "ERROR: could not generate big.var and big.av test files" << std::endl;
        EXPECT_EQ(result, 0);
    }
    ...
    */
    ...
    sprintf(out_filename,  "%s%s", input_test_dir, "big_save.var");
    start = std::chrono::high_resolution_clock::now();
    ...
    // ==== delete big_save.var to save disk space ====
    success = remove(out_filename);
    if(success == 0)
        std::cout << "File '" << out_filename << "' deleted successfully" << std::endl;
    else
    ...
    // **** K_save() performance tests ****
    memset(out_filename, 0, sizeof(out_filename));
    sprintf(out_filename,  "%s%s", input_test_dir, "big_save.av");
    start = std::chrono::high_resolution_clock::now();
    K_save(KV_WS, out_filename);
    end = std::chrono::high_resolution_clock::now();
    elapsed = end - start;
    ...
    // ==== delete big_save.av to save disk space ====
    success = remove(out_filename);
    if(success == 0)
        std::cout << "File '" << out_filename << "' deleted successfully" << std::endl;
    else
