...
int     port;
{
    ...
    if((WinSocket = socket(AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0) {   /* BP_M 17-02-2008 */
...
//    if(txt[0] == 0) return(0);  /* JMP 05-11-01 */
    if(lg <= 0) lg = strlen(txt) + 1;     // ???!!!??? ProblŠme pour les strings NULLS
...
WSockDebugBuf(unsigned char *buf, int lg)
{
    int     j;
...
char    *a1, *a2, *a3, *a4, *a5, *a6, *a7, *a8, *a9;
{
    ...
    return(WSockBWrite(cltsock, buf, strlen(buf)));
}
...
{
    int     rc, cursock, nb_try = 0, i, moderecv, nb_try2 = 0;
    int     tmp_check_read = -1;
    unsigned long imode = 1;
    char    buf[512], *msg_txt = 0;
