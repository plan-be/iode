...
On passe en paramŠtre: le pointeur sur la matrice … ‚diter, un pointeur sur
...
Les titres des lignes et colonnes sont pass‚s dans deux tableaux de chaŒnes
dans lesquels on trouve tous les titres des lignes et tous les titres de
colonnes dans l'ordre, ces tableaux de chaŒnes doivent ˆtre termin‚s par
NULL.

Les titres par d‚faut sont les num‚ros de lignes et de colonnes.

Les valeurs lc0 et lc repr‚sentent les largeurs des premiŠre et autres
colonnes de la matrice en caractŠres.
...
Les touches d'‚dition suivantes sont d‚finies:
   ...
   - TAB et Shift-TAB : d‚place le curseur d'un ‚cran vers la droite ou
     la gauche
   - HOME : place le curseur en premiŠre colonne de la ligne courante
   - CTRL + HOME : place le curseur en premiŠre ligne de la colonne courante
   ...
   - Alt-M : D‚place le tableau dans les limites de l'‚cran suivant
     les touches curseur utilis‚es ensuite. L'‚dition reprend aprŠs que
     ENTER ait ‚t‚ press‚.
