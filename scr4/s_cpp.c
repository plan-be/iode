...
/* ================================================================
Ouvre un fichier en mode interpr‚tation. L'interpr‚teur gŠre les
...
Trois variables contenant les caractŠres sp‚ciaux reconnus par les
fonctions du groupe Cpp peuvent ˆtre (re-)d‚finies … tout moment.
...
/* ================================================================
Ferme le fichier pr‚alablement ouvert par CppOpen() et libŠre les
buffers associ‚s.
...
&CO
    ...
    2 : B  (LIFO)
    3 : A  (LIFO)
    4 : 1  (Premier caractŠre de la chaŒne)
    5 : 2  (deuxiŠme)
    6 : 3  (troisiŠme)
    7 : 4  (quatriŠme)
&TX
...
/* ================================================================
Place le caractŠre ch dans le stream du fichier cpp ouvert.
La lecture suivante retournera ce caractŠre.

Contrairement … la fonction ungetc(), plusieurs CppUngetc() peuvent
ˆtre appel‚s successivement. Les caractŠres seront retourn‚s selon
la m‚thode LIFO comme pour CppRecord().

&EN cpp = descripteur du fichier ouvert
&EN ch = caractŠre … enregistrer
...
/* ================================================================
Lit le caractŠre suivant dans le fichier d‚crit par cpp. Ce caractŠre
...
par la fonction CppGetc() et le caractŠres retourn‚ d‚pend des actions
d‚termin‚es par les directives.

Trois cas sont … prendre en consid‚ration :

&EN La directive include (commen‡ant par le caractŠre d‚fini dans
    CPP_DIRCH) en d‚but de ligne. Le fichier indiqu‚ est ouvert et
    le premier caractŠre du sous-fichier est retourn‚ (sauf si
...
&EN La directive define (commen‡ant par le caractŠre d‚fini dans
    CPP_DIRCH) en d‚but de ligne. La d‚finition est lue et s'ajoute … la
    table des defines. Le caractŠre suivant dans le fichier est retourn‚
    par la fonction.

&EN Un define (commen‡ant par le caractŠre d‚fini dans CPP_DEFCH). Le
    caractŠre retourn‚ est le premier de la valeur d‚finie du define.
...
&RT Le caractŠre suivant dans le fihier cpp.
...
/*NH ================================================================
LibŠre l'espace allou‚ par les define.
...
CppFreeDefines(). La fonction CppClose() ne libŠre pas cet espace.
...
et le caractŠre courants. Le format est :
