...
#endif
{
    ...
    ao->ao_type = type;
    switch(type) {
	case A2M_PAR   : ao->ao_ptr = (char *)A2mNewPar(0L); break;
	case A2M_TBL   : ao->ao_ptr = (char *)A2mNewTbl(); break;
	case A2M_GRF   : ao->ao_ptr = (char *)A2mNewGrf(); break;
	case A2M_TOPIC :
	case A2M_TOPICALIAS: ao->ao_ptr = (char *)A2mNewTopic(); break;
	case A2M_PAGE  : ao->ao_ptr = (char *)A2mNewPage(); break;
	case A2M_LINE  : ao->ao_ptr = (char *)A2mNewLine(); break;
	case A2M_GOTO  : ao->ao_ptr = (char *)A2mNewGoto(); break;
	case A2M_MARGINS : ao->ao_ptr = (char *)A2mNewMargins(); break;
	default : break;
	}
    return(ao);
}
...
A2MOBJ  *ao;
{
    if(ao == 0) return(0);
    switch(ao->ao_type) {
	case A2M_PAR : A2mFreePar((A2MPAR *)ao->ao_ptr); break;
	case A2M_TBL : A2mFreeTbl((A2MTBL *)ao->ao_ptr); break;
	case A2M_GRF : A2mFreeGrf((A2MGRF *)ao->ao_ptr); break;
	case A2M_TOPIC :
	case A2M_TOPICALIAS : A2mFreeTopic((A2MTOP *)ao->ao_ptr); break;
	case A2M_PAGE : A2mFreePage((A2MPAGE *)ao->ao_ptr); break;
	case A2M_LINE : A2mFreeLine((A2MLINE *)ao->ao_ptr); break;
	default : break;
	}
    SCR_free(ao);
    return(0);
...

{
    ...
    al->al_type = type;
    if(type == 0) al->al_type = CppReadLong(af->af_cpp);
    CppSkipToEol(af->af_cpp);
    return(ao);
}
...
#endif
{
    ...
    A2M_CURTOPIC = atp->atp_nb = CppReadLong(af->af_cpp);
    atp->atp_level = CppReadLong(af->af_cpp);
    CppSkipSpaces(af->af_cpp);
    ...
    CppSkipToEol(af->af_cpp);
    atp->atp_lg = strlen(atp->atp_keyws);
    return(ao);
}
...
#endif
{
    ...
    /* .tb 12 */
    at->at_nc = CppReadLong(af->af_cpp);
	...
	if(ch == A2M_CMDCH) {
	    ...
	    else if(strcmp("tbody", str) == 0)   {pos = 1; CppSkipToEol(af->af_cpp);}
	    else if(strcmp("theader", str) == 0) {pos = 0; CppSkipToEol(af->af_cpp);}
	    else if(strcmp("tfooter", str) == 0) {pos = 2; CppSkipToEol(af->af_cpp);}
	    else if(strcmp("tspaceb", str) == 0) {at->at_tspaceb = A2mReadInt(af);}  /* JMP 15-03-2004 */
	    else if(strcmp("tspacea", str) == 0) {at->at_tspacea = A2mReadInt(af);}  /* JMP 15-03-2004 */
...
int     axis;
{
    A2MGXY  *gxy;
    U_ch    str[256], ch, *txt;
    int     nl = SCR_tbl_size(*gxys), nxy = 0;
    double  xy[2];
/*
#ifndef WATCOM
    double  atof();
...
*/
    switch(A2M_GALIGN) {
	case 'L' : align = 0.0; break;
	case 'M' : align = skip/2; break;
	case 'R' : align = skip; break;

	default  : align = 0.0; CppUngetc(af->af_cpp, ch);
    }
...
A2MGRF  *ag;
{
    ...
    ag->ag_box = CppReadLong(af->af_cpp);
    CppSkipSpaces(af->af_cpp);
...
#endif
{
    ...
    else if(strcmp(str, "page")  == 0) {return(A2mReadPage(af));}
    else if(strcmp(str, "line")  == 0) {return(A2mReadLine(af, 0));}
    else if(strcmp(str, "line1") == 0) {return(A2mReadLine(af, 1));}
    else if(strcmp(str, "line2") == 0) {return(A2mReadLine(af, 2));}
    else if(strcmp(str, "tb")    == 0) {return(A2mReadTbl(af));}
    else if(strcmp(str, "gb")    == 0) {return(A2mReadGraph(af));}
    else if(strcmp(str, "topic") == 0) {return(A2mReadTopic(af));}
    else if(strcmp(str, "topicalias") == 0) {return(A2mReadTopicAlias(af));}
    else if(strcmp(str, "goto") == 0) {return(A2mReadGoto(af));}
    else if(strcmp(str, "margins") == 0) {return(A2mReadMargins(af));}
    else {A2mPrintError(af, "Unrecognized command"); return(ao);}
}
...
char    str[51];
{
    ...
    if(strcmp(str, "cmd") == 0)         {A2mDefCh(af, &A2M_CMDCH); CPP_DIRCH = A2M_CMDCH; return(1);}
    else if(strcmp(str, "esc")    == 0) {A2mDefCh(af, &A2M_ESCCH); return(1);}
    else if(strcmp(str, "def")    == 0) {A2mDefCh(af, &A2M_DEFCH); CPP_DEFCH = A2M_DEFCH; return(1);} /* JMP 08-05-97 */
    else if(strcmp(str, "sep")    == 0) {A2mDefCh(af, &A2M_SEPCH); return(1);}
    else if(strcmp(str, "ignore") == 0) {A2mReadVar(af, &A2M_IGNORECHARS); return(1);}
    else if(strcmp(str, "par")    == 0) {A2mDefPar(af, A2M_CURTAG); return(1);}
    else if(strcmp(str, "font")   == 0) {A2mDefPar(af, A2M_CURCTAG); return(1);}
    else if(strcmp(str, "par1")   == 0) {A2mDefPar(af, A2M_CURTAG1); return(1);}
    else if(strcmp(str, "lf_on")  == 0) {A2M_LFON = 1; return(1);}
    else if(strcmp(str, "lf_off") == 0) {A2M_LFON = 0; return(1);}
    else if(strcmp(str, "bl_on")  == 0) {A2M_BLON = 1; return(1);}
    else if(strcmp(str, "bl_off") == 0) {A2M_BLON = 0; return(1);}
    else if(strcmp(str, "pghead") == 0) {A2mReadVar(af, &A2M_PGHEAD); return(1);}
    else if(strcmp(str, "pgfoot") == 0) {A2mReadVar(af, &A2M_PGFOOT); return(1);}
    else if(strcmp(str, "tshading") == 0) {A2mReadTShading(af); return(1);}
    else if(strcmp(str, "tbreak") == 0) {A2M_TBREAK = A2mReadInt(af); return(1);}
    else if(strcmp(str, "tpage") == 0)  {A2M_TPAGE = A2mReadInt(af); return(1);}
    else if(strcmp(str, "twidth") == 0) {A2mReadTblWidths(af); return(1);}
    else if(strcmp(str, "gpage") == 0)  {A2M_GPAGE = A2mReadInt(af); return(1);}
    else if(strcmp(str, "pardef") == 0)  {A2mReadParDef(af); return(1);}
    else if(strcmp(str, "topiclink") == 0)  {A2mReadTopicLink(af, (A2MTOP *)0); return(1);}
    else if(strcmp(str, "topicslink") == 0) {A2mReadTopicsLink(af); return(1);}
    else if(strcmp(str, "bminidoc")    == 0) {A2mReadAnchor(af, 0); return(1);}
    else if(strcmp(str, "imgpath")    == 0) {A2mReadVar(af, &A2M_IMGPATH); return(1);} /* JMP 24-09-10 */
    else return(0);
}
...
#endif
{
	...
	chattr = 1;
	switch(txt[pos + 1]) {
	    case 'b' : as_proto.as_fnt.af_bold        = 1; break;
	    case 'B' : as_proto.as_fnt.af_bold        = 0; break;
	    case 'c' : as_proto.as_fnt.af_family      = 'C'; break;
	    case 'C' : as_proto.as_fnt.af_family      = 0; break;    /* JMP 26-08-99 */
	    case 'g' : as_proto.as_fnt.af_family      = 'G'; break;  /* JMP 26-08-99 */
	    case 'G' : as_proto.as_fnt.af_family      = 0; break;
	    case 'i' : as_proto.as_fnt.af_italic      = 1; break;
	    case 'I' : as_proto.as_fnt.af_italic      = 0; break;
	    case 'u' : as_proto.as_fnt.af_underline   = 1; break;
	    case 'U' : as_proto.as_fnt.af_underline   = 0; break;
	    case 'o' : as_proto.as_fnt.af_strike      = 1; break;
	    case 'O' : as_proto.as_fnt.af_strike      = 0; break;
	    ...
	    case 'e' : as_proto.as_fnt.af_bold        = 0;
		       as_proto.as_fnt.af_italic      = 0;
		       as_proto.as_fnt.af_underline   = 0;
		       as_proto.as_fnt.af_strike      = 0;
		       break;

	    case 's' : as_proto.as_fnt.af_asize       -= 2; break;
	    case 'S' : as_proto.as_fnt.af_asize       += 2; break;

	    case '-' : as_proto.as_fnt.af_pos         = -1;
		       as_proto.as_fnt.af_asize       -= 2;
		       break;
	    case '+' : as_proto.as_fnt.af_pos         = 1;
		       as_proto.as_fnt.af_asize       -= 2;
		       break;
	    case '=' : as_proto.as_fnt.af_pos         = 0;
...
fichier et la ligne et la colonne dans ce fichier o— en est arriv‚e la
...
/*NH*/
A2mHHCalcPosA(txt, shift, pos, posH)
U_ch    *txt;
int     shift, pos, *posH;
{
    int     i, ch;
...
A2mFreeFNotes()
{
    ...
    SCR_add_ptr(&A2M_FNOTES, &A2M_NB_FNOTES, 0L);
    SCR_free_tbl(A2M_FNOTES);
...
char    *ref;
{
    SCR_add_ptr(&A2M_FNOTES, &A2M_NB_FNOTES, ref);
}
