...
&EN usection vaut 1 si les titres des sections doivent ˆtre
    automatiquement transpos‚s en majuscules.
&EN uparam indique la mˆme chose pour les lignes
    de paramŠtres internes aux sections.
...
LibŠre la structure INIFILE allou‚e.
...
/* ====================================================================
Ouvre le fichier filename du type .ini et se positionne sur la premiŠre
...
/* ====================================================================
Ouvre le fichier filename du type .ini et se positionne sur la premiŠre
section dont le nom est section. Le nom de la section doit ˆtre
fourni en majuscules.

Tous les paramŠtres de la section sont lus et stock‚s dans un tableau
...
INIFILE inif. Les lignes de paramŠtres sont retourn‚es dans un tableau
...
/* ====================================================================
Lit dans le fichier filename la valeur du paramŠtre parm de la section
section. La d‚finition du paramŠtre est stock‚ dans le buffer val dont
la longueur est au minimum de lg caractŠres.

&EN filename = nom du fichier .ini
&EN section = section (en majuscules) dont le paramŠtre dit ˆtre lu
&EN parm = paramŠtre … lire
&EN val = pointeur destin‚ … contenir la valeur du paramŠtre
&EN lg = longueur maximum du paramŠtre

&RT 0 si le paramŠtre est trouv‚, -1 sinon
...
/* ====================================================================
Lit dans le fichier filename la valeur du paramŠtre parm de la section
section. La d‚finition du paramŠtre est stock‚ dans l'entier dont le
...
/* ====================================================================
Lit dans le fichier filename la valeur du paramŠtre parm de la section
section. La d‚finition du paramŠtre est stock‚ dans le double dont le
pointeur est pass‚ dans val.
...
&RT 1 si la valeur du paramŠtre est yes, oui, ja ou 1 (en minuscules)
...
/* ====================================================================
Remplace dans un fichier .ini la valeur d'un paramŠtre.
...
&EN -2 en cas de non remplacement (paramŠtre non trouv‚)
...
char    *filename, *section, *parm, *val;
{
	...
	if(IniIsSection(buf, section)) {
		...
		if(IniIsParm(buf, parm)) {
		    /* C'est le bon paramŠtre : on le remplace */
		    /* ISC_fprintf(fdo, "%s=%s\n", parm, val); */
		    sprintf(buf, "%s=%s\n", parm, val);
		    ISC_fwrite(buf, strlen(buf), 1, fdo);
		    ok = 0;
		    ...
		    /* C'est un autre paramŠtre : on continue */
		    /* ISC_fprintf(fdo, "%s\n", bufo);       */
		    sprintf(buf, "%s\n", bufo);
		    ISC_fwrite(buf, strlen(buf), 1, fdo);
		    }
