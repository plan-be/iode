...
static CLEC* link_lec(const std::string& lec)
{
    ...
    int res = L_link(global_ws_var.get(), global_ws_scl.get(), clec);
    if(res != 0)
    {
        SCR_free(clec);
        kerror(0, error_msg);
...
double execute_lec(const std::string& lec, const int t)
{
    ...
    double value = L_exec(global_ws_var.get(), global_ws_scl.get(), clec, t);
    SCR_free(clec);
...
std::vector<double> execute_lec(const std::string& lec)
{
    ...
    for(int t = 0; t < nb_per; t++) 
        res.push_back(L_exec(global_ws_var.get(), global_ws_scl.get(), clec, t));
    SCR_free(clec);
