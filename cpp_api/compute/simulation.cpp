...
bool Simulation::model_compile(const std::string& list_eqs)
{
    ...
    {        
        ...
        else 
        {
            try
            {
                std::shared_ptr<KDBEquations> tdbe = new KDBEquations(global_ws_eqs, list_eqs, false);
                if(tdbe->size() > 0)
                    rc = KE_compile(tdbe);
                delete tdbe;
            }
    ...
    const std::string& list_eqs)
{
    ...
    int rc = -1;
    if(list_eqs.empty())
        rc = K_simul(global_ws_eqs, global_ws_var, global_ws_scl, 
    ...
    else 
    {
        try
        {
            ...
            if(tdbe->size() > 0)
            {
                ...
                rc = K_simul(tdbe, global_ws_var, global_ws_scl, sample, 
                                KSIM_EXO, c_eqs);
    ...
    const std::string& pre_name, const std::string& inter_name, const std::string& post_name)
{
    ...
    try
    {
        std::shared_ptr<KDBEquations> tdbe = new KDBEquations(global_ws_eqs, list_eqs, false);
        if(tdbe->size() > 0)
            rc = K_simul_SCC(tdbe, global_ws_var, global_ws_scl, sample, 
