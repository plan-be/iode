...
char *HLP_filename(char *filename)
{
	...
	//PG_display_error("file : %s", HLP_FILENAME);
	if(access(HLP_FILENAME, 0)) {
		SCR_free(HLP_FILENAME);
		HLP_FILENAME = SCR_malloc(strlen("c:\\iode\\") + strlen(file) + 2);
		sprintf(HLP_FILENAME, "c:\\iode\\%s", file);
		//PG_display_error("file : %s", HLP_FILENAME);
		}
	if(access(HLP_FILENAME, 0)) {
		sprintf(HLP_FILENAME, "%s", file);
		//PG_display_error("file : %s", HLP_FILENAME);
		}
