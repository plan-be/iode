...
from PyQt6.QtGui import (QPaintEvent, QResizeEvent, QFontMetricsF, QPainter, QShortcut, 
                         QKeySequence, QTextBlock, QTextCursor)
from PyQt6.QtCore import Qt, QRect, QSize, QPoint, QSettings
...
class IodeTextEditor(IodeAutoCompleteTextEdit):
    ...
    def __init__(self, parent=None):
        ...
        self.block_count_changed.connect(self.update_left_area_width)
        self.update_request.connect(self.update_left_area)

        # set tab = 4 whitespaces
        self.set_tab_stop_distance(QFontMetricsF(self.font()).horizontalAdvance(' ') * 4)
    ...
    def popup_find_replace_box(self, find_only: bool=True):
        ...
        self.find_replace_dialog.setTextToFind(cursor.selectedText())
        # set if replace part is showed
        self.find_replace_dialog.findOnly(find_only)
