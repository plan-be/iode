...
from utils import IODE_REPORT_EXTENSION
...
class ReportWidget(AbstractTextWidget):
    ...
    def __init__(self, filepath: str, main_window: AbstractMainWindow, parent: QWidget=None):
        ...
        self._editor_2.setDocument(self.editor.document())

        # Setup the editors
        self._editor.setup()
        self._editor_2.setup()

        self._editor_2.hide()

        q_langs = [member.name for member in TableLang]
        self.ui.comboBox_language.addItems(q_langs)
    ...
    def _load(self, filepath, force_overwrite):
        ...
        :param filepath: The path to the report file.
        :param force_overwrite: Whether to force overwrite the current report.
        """
        return self._load_(filepath, force_overwrite)
    ...
    def _run(self):
        ...
        parameters = self.ui.lineEdit_parameters.text()
        language = TableLang(self.ui.comboBox_language.currentIndex())
        nb_decimals = self.ui.spinBox_nbDecimals.value()
