...
from iode import (IodeTypes, IodeFileType, FileType, IODE_FILE_TYPES, _IODE_DEFAULT_DATABASE_FILENAME, 
                  Equation, Identity, comments, equations, identities, lists, scalars, tables, variables, 
                  _check_filepath) 
...
class IodeAbstractTableModel(QAbstractTableModel):
    ...
    def load(self, filepath: str, force_overwrite: bool):
        ...
        try:
            # NOTE: _check_filepath() converts to absolute path
            filepath = _check_filepath(filepath, iode_file_type, file_must_exist=True)
    ...
    def save(self, project_dir: QDir, filepath: str=None) -> str:
        ...
        if not filepath or filepath == _IODE_DEFAULT_DATABASE_FILENAME:
        ...
        # NOTE: _check_filepath() converts to absolute path
        filepath = _check_filepath(filepath, iode_file_type, file_must_exist=False)
    ...
    def compute_hash(self, before=False):
        ...
        if before:
            self.hash_before = hash_value(self._database)
        else:
            self.hash_after = hash_value(self._database)
            if self.hash_after != self.hash_before:
                self.database_modified.emit()

    def data_cell(self, row: int, col: int) -> Any:
        """
    ...
    def _ask_filepath_dialog(self, project_dir: QDir) -> str:
        ...
        default_filename = _IODE_DEFAULT_DATABASE_FILENAME + file_type.extensions[0]
