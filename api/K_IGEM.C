...
ag:
    switch(YY_lex(yy)) {
	case YY_DOUBLE : val = minus * yy->yy_double; break;
	case YY_LONG   : val = minus * yy->yy_long; break;
	case YY_SPECIAL:
	    if(yy->yy_text[0] == '-') {
		minus = -1;
		goto ag;
	...
	case YY_WORD   : if(strcmp("na", yy->yy_text) != 0)
			     YY_unread(yy);
			 return(L_NAN);
...
int     dim; IODE_REAL    *vector;
{
	...
	GEM_mat = SCR_malloc(GEM_nser * GEM_nobs * sizeof(IODE_REAL));
