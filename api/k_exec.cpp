...
static int KI_read_vars_db(KDBVariables* dbv, KDBVariables* dbv_tmp, char* source_name)
{
    ...
    for(const auto& [name, var] : dbv->k_objs)
    {
    ...
    for(const std::string& name : vars_to_read)
    {
        ...
        if(dbv->contains(name)) 
            continue;

        // create the VAR in dbv
        Variable var(vsmpl->nb_periods, IODE_NAN);

        // copy the VAR from dbv_tmp to dbv
        Variable& tmp_var = dbv_tmp->get_obj(name);
        for(int t = 0; t < smpl.nb_periods; t++)
            var[start + t] = tmp_var[start_tmp + t];
    ...
    int nb, char* files[])
{
    ...
    if(nb_found < dbv->size()) 
    {
        ...
        std::map<std::string, Variable> k_objs_copy = dbv->k_objs;
        for(const auto& [name, var] : k_objs_copy) 
        {
...
static int KI_read_scls_db(KDBScalars* dbs, KDBScalars* dbs_tmp, char* source_name)
{
    ...
    for(const std::string& name : dbs->get_names()) 
    {
        ...
        dbs->copy_obj_from(dbs_tmp, name, name);
...
static int KI_read_scls_file(KDBScalars* dbs, char* file)
{
    ...
    for(const auto& [name, scl_ptr] : dbs->k_objs) 
    {
