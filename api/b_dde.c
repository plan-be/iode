...
char *IodeDdeCreateObj(int objnb, int type, int *nc, int *nl)
{
	...
	*nl = 1;
	switch(type) {
	    case K_CMT : obj = (char *)KCVAL(kdb, objnb); break;
	    case K_EQS : obj = (char *)KELEC(kdb, objnb); break;
	    case K_IDT : obj = (char *)KILEC(kdb, objnb); break;
	    case K_LST : obj = (char *)KLVAL(kdb, objnb); break;
	    default    : obj = SCR_stracpy("Not yet implemented") ; break;
	}

	if(obj == 0) obj = " ";
	res = SCR_malloc(sizeof(ONAME) + 10 + strlen(obj));
	strcpy(res, KONAME(kdb, objnb));
	strcat(res, "\t");
	strcat(res, obj);
	/* strcat(res, "\t");*/
...
char *IodeDdeGetXObj(char *szItem, int type)
{
    ...
    char    *res = 0, *sheet, *item,
	    *plst =  NULL, *K_expand();
    KDB     *kdb;
    int     objnb, line, col, i, j, pos, l, h;
    HCONV   hConv;
...
IodeDdeSetWS(char *szItem, char *szBuffer)
{
    KDB     *kdb;
    int     i, type, rc = 0;
...
IodeDdePlay(char *szItem, char *szBuffer)
{
    U_ch    **keys;
    int     key, i, n;
...
IodeDdeSetItem(char *szTopic, char *szItem, char *szBuffer)
{
    char    *tmp, buf[30];
    KDB     *kdb;
    int     type;
    ...
    tmp = SCR_malloc(strlen(szBuffer) + 30);
    if(type == K_VAR) {
...
B_ExcelSetItem(char *ddeitem, char *ptr, int nc, int nl)
{
...
B_ExcelLang(char *arg)
{
	...
	if(nb_args == 1) {
		switch(args[0][0]) {
			case 'f':
			case 'F':       ExcelCellRef = ExcelCellFr;
						ExcelSheetRef = ExcelSheetFr;
						break;
			case 'n':
			case 'N':       ExcelCellRef = ExcelCellNl;
						ExcelSheetRef = ExcelSheetNl;
						break;
			default:        ExcelCellRef = ExcelCellEn;
...
B_ExcelSet(char *arg, int type)
{
    int     i, pos, shift, lg, rc = -1,
    ...
    item = args[nb_args - 1];
    switch(type) {
	case K_CMT : ptr = SCR_stracpy(KCVAL(kdb, pos)); break;
	case K_IDT : ptr = SCR_stracpy(KILEC(kdb, pos)); break;
	case K_LST : ptr = SCR_stracpy(KLVAL(kdb, pos)); break;
	case K_EQS : ptr = SCR_stracpy(KELEC(kdb, pos)); break;
	case K_SCL :
...
B_ExcelExecute(char *arg)
{
    int     rc;
    char    *cmd;

    SCR_strip(arg);
    cmd = SCR_malloc(strlen(arg)+2);
    sprintf(cmd, "[%s]", arg);
...
B_ExcelCmd(char *cmd, char *arg)
{
    ...
    if(arg != NULL && arg[0] != '\0') {
	xlscmd = SCR_malloc(strlen(cmd) + strlen(arg) + 10);

	if(U_is_in('"', arg)) sprintf(xlscmd, "[%s(%s)]", cmd, arg);
	else                  sprintf(xlscmd, "[%s(\"%s\")]", cmd, arg);
    }
    else {
	xlscmd = SCR_malloc(strlen(cmd) + 2);
