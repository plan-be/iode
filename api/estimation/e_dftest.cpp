...
static int E_UnitRoot_1(Sample* smpl, char* buf)
{
    ...
    Estimation est(eqs, global_ws_eqs, global_ws_var.get(), global_ws_scl, smpl);
    rc = est.estimate();
...
double *E_UnitRoot(char* lec, int drift, int trend, int order)
{
    ...
    vec = L_cc_link_exec(lec, global_ws_var.get(), global_ws_scl);
    if(vec == NULL) 
        return NULL;
    strcpy(varname, "_DF");
    ...
    global_ws_scl->set("df_", scalar);
    
    if(drift) 
    {
        sprintf(buf + strlen(buf), "+ df_d");
        global_ws_scl->set("df_d", scalar);
    }

    if(trend) 
    {
        sprintf(buf + strlen(buf), "+ df_t*t");
        global_ws_scl->set("df_t", scalar);
    }

    for(i = 1 ; i <= order ; i++) 
    {
        sprintf(scl, "df%d", i);
        global_ws_scl->set(scl, scalar);
    }
