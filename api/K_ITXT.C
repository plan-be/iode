...
int     *shift; IODE_REAL    *dval;
{
    ...
    SCR_strlcpy(y, date, 4); y[4] = 0; ly = atoi(y);        /* JMP 13-02-2013 */
    SCR_strlcpy(m, date + 4, 2); m[2] = 0; lm = atoi(m);    /* JMP 13-02-2013 */

    switch(TXT_freq) {
	case 'Q' : lm = 1 + lm/4; break; /* JMP 05-10-00 */
	case 'Y' : lm = lm/12; break;
    }
...
char    **cmt;
{
    ...
    SCR_sqz(tbl[5]);
    SCR_sqz(tbl[6]);
    len = strlen(tbl[TXT_lang]) + strlen(tbl[5]) + strlen(tbl[6]) + 10;
    *cmt = SW_nalloc(len);
