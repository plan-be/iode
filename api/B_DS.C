...
char    *B_DSPeriod2Date(PERIOD *per, char *date, char *freq)
{
    ...
    *freq = per->p_p;
    switch(per->p_p) {
	case 'Y': sprintf(date, "1/1/%s", year); break;
	case 'Q': sprintf(date, "1/%ld/%s", 3 * per->p_s, year); break;
	case 'M': sprintf(date, "1/%ld/%s", per->p_s, year); break;
    }
...
B_DSDate2Period(PERIOD *per, char *date, char freq)
{
    char    **tbl;
    extern  long    atol();
	...
	switch(freq) {
	    case 'Y' : break;
	    case 'Q' : per->p_s = atol(tbl[1])/4; break;
	    case 'M' : per->p_s = atol(tbl[1]);   break;
	}
	SCR_free_tbl(tbl);
	return(0);
    }
	...
	switch(freq) {
	    case 'Y' : break;
	    case 'Q' : per->p_s = atol(tbl[0] + 1); break;
	    case 'M' : per->p_s = atol(tbl[0] + 1)* 3;   break;
	}
	SCR_free_tbl(tbl);
	return(0);
    }
...
B_DSUpdateCmt(char *name, char *val)
{
    int     rc = -1;
    char    *update;

    update = SCR_malloc(K_MAX_NAME + 3 + strlen(val)); /* IODE64K */
    if(update) {
...
B_DSUpdateVar(char *name, PERIOD *per, char *val)
{
    int     rc = -1;
    char    *update, period[11];

    update = SCR_malloc(K_MAX_NAME + 23 + strlen(val)); /* IODE64K */
    if(update) {
	SCR_replace(val, ",", ".");
...
B_DSInterpret(char *code, char freq, char *contents)
{
...
B_DSImportDb_1(char *arg, SAMPLE *smpl)
{
    char            freq;
    unsigned char   *ptr,
		    request[81],
...
B_DSImportDb(char *arg)
{
    ...
    return(B_ainit_loop(arg, B_DSImportDb_1, smpl));
}
//#endif
