...
{
	YYFILE	*yy;
	char	lhs[EV_MAXBUF + 1], scl[80], word[256];
...
int EV_cc_file(char *filename)
{
	...
	while(1) {
		...
		if(memcmp(buf, EV_ESTSUBCOEFS, strlen(EV_ESTSUBCOEFS)) == 0) {
			...
			if(nbcoefs < 0) break;
			// Save in WS
			if(B_DataUpdateEqs(endo, lec, lasteq, 0, 0L, 0L, 0L, 0L, 0)) break;
			for(i = 0 ; i < nbcoefs ; i++) {
        ...
        if(EV_INIDENTITIES || memcmp(buf, EV_IDENTITY, strlen(EV_IDENTITY)) == 0) {
            ...
            // Save in WS
            if(B_DataUpdateEqs(endo, lec, buf, 0, 0L, 0L, 0L, 0L, 0)) {
