 ...
 *  @return      char*           pointer to *code
 */
char* ExportObjsCSV::extract_comment(std::shared_ptr<KDBComments> dbc, char* name, char**cmt)
{
 ...
 *  @return          char*          *vec
 */
char* ExportObjsCSV::get_variable_value(std::shared_ptr<KDBVariables> dbv, int nb, int t, char** vec)
{
    int     lg, olg;
...
char* ExportObjsRevertCSV::get_variable_value(std::shared_ptr<KDBVariables> dbv, int nb, int t, char** vec)
{
    char  tmp[81]; 
    char* buf = NULL;
