...
int B_DataCreate_1(char* arg, int* ptype)
{
    try 
    {
        ...
        switch(*ptype) 
        {
            ...
            {
                Comment comment("");
                global_ws_cmt->set(name, comment);
                break;
            }
            case EQUATIONS :
            {
                ...
                Equation eq(name, lec, EQ_LSQ, "", "", "", "", "", false);
                global_ws_eqs->set(name, eq);
                break;
            }
            case IDENTITIES :
            {
                Identity idt(name);
                global_ws_idt->set(name, idt);
                break;  
            }
            case LISTS :
            {
            ...
            {
                Scalar scl;
                global_ws_scl->set(name, scl);
                break;
            }
            case TABLES :
            {
                Table tbl(2, "TITLE", "LEC", false, false, false);
                global_ws_tbl->set(name, tbl);
                break;
            }
            case VARIABLES :
            {
