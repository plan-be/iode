...
public:
        ...
        const std::string& dest_key) const
    {
        ...
        std::shared_ptr<T> obj_copy_ptr = std::make_shared<T>(*obj_ptr);
    ...
    virtual void set_obj_ptr(const std::string& name, std::shared_ptr<T> obj_ptr)
    {
        std::string key = to_key(name);
        if(key.empty())
        {
        ...
        // Share the same object instance - no copy is made
