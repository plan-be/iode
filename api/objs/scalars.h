...
public:
    Scalar(const double value=0.9, const double relax=1.0, const double std=IODE_NAN)
    {
        this->value = value;
        this->relax = relax;
        this->std = std;
...
struct KDBScalars : public KDBTemplate<Scalar>
{
    ...
    KDBScalars(const std::shared_ptr<KDBScalars> db_parent, const std::string& pattern, const bool copy) 
