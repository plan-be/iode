...
int   K_epack(char **pack, char *a1, char *endo)
{
    ...
    *pack = (char*) P_add(*pack, (char*) eq->lec.c_str(), eq->lec.size());          /* lec */
    ...
    *pack = (char*) P_add(*pack, (char*)&(eq->sample), sizeof(Sample));     /* sample */
    ...
    if(eq->comment.empty()) 
        ...
        *pack = (char*) P_add(*pack, (char*) eq->comment.c_str(), eq->comment.size());          /* cmt */

    if(eq->block.empty()) 
        *pack = (char*) P_add(*pack, NULL, 1);
    else 
        *pack = (char*) P_add(*pack, (char*) eq->block.c_str(), eq->block.size());              /* blk */

    if(eq->instruments.empty()) 
        *pack = (char*) P_add(*pack, NULL, 1);
    else 
        *pack = (char*) P_add(*pack, (char*) eq->instruments.c_str(), eq->instruments.size());  /* instr */
