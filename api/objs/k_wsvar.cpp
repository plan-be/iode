...
int KV_merge(KDBVariablesPtr kdb1, KDBVariablesPtr kdb2, int replace)
{
    ...
    int start1, start2;
    Sample smpl = kdb1->get_sample()->intersection(*kdb2->get_sample());
    if(smpl.nb_periods > 0) 
    {
        start1 = smpl.start_period.difference(kdb1->get_sample()->start_period); /* always >= 0 */
        start2 = smpl.start_period.difference(kdb2->get_sample()->start_period); /* always >= 0 */
    }
...
double KV_get(const KDBVariablesPtr kdb, const std::string& name, int t, int mode)
{
    ...
    std::string error_msg = "Cannot get the value of the variable named '" + name + "' ";
    error_msg += "at period position " + std::to_string(t) + ":\n";
    
    if(!kdb->contains(name))
    {
        error_msg += "Variable not found in the database.";
    ...
    if(t < 0 || t >= smpl->nb_periods)
    {
        error_msg += "invalid period index " + std::to_string(t) + ". Valid values are between 0 and ";
        error_msg += std::to_string(smpl->nb_periods - 1) + ".";
        kwarning(error_msg.c_str());
        return IODE_NAN;
    }
    ...
    switch(mode) 
    {
        ...
        case VAR_MODE_Y0Y_DIFF : 
            ...
            vt = var[t];
            vt_shifted = var[t - pernb];
            if(IODE_IS_A_NUMBER(vt) && IODE_IS_A_NUMBER(vt_shifted))
                value = vt - vt_shifted;
            else 
                value = IODE_NAN;
            break;
        ...
        case VAR_MODE_Y0Y_GROWTH_RATE :
            ...
            vt = var[t];
            vt_shifted = var[t - pernb];
            if(IODE_IS_A_NUMBER(vt) && IODE_IS_A_NUMBER(vt_shifted) && !IODE_IS_0(vt_shifted))
                value = (vt / vt_shifted - 1) * 100.0;
            else 
                value = IODE_NAN;
            break;
    }
...
void KV_set(KDBVariablesPtr kdb, const std::string& name, int t, int mode, double value)
{
    ...
    std::string error_msg = "Cannot set the value of the variable named '" + name + "' ";
    error_msg += "at period position " + std::to_string(t) + ":\n";
...
Variable KDBVariables::get(const std::string& name) const
{
	...
	vars.reserve(nb_obs);
    KDBVariablesPtr self = const_cast<KDBVariables*>(this)->shared_from_this();
	for(int i=0; i < nb_obs; i++) 
		vars.push_back(KV_get(self, name, i, 0));
	return vars;
}
...
double KDBVariables::get_var(const std::string& name, const int t, const IodeVarMode mode) const
{
    ...
    KDBVariablesPtr self = const_cast<KDBVariables*>(this)->shared_from_this();
    return KV_get(self, name, t, mode);
}
...
void KDBVariables::set_var(const std::string& name, const int t, const double value, const IodeVarMode mode)
{
	...
	KV_set(shared_from_this(), name, t, mode, value);
}
...
Variable KDBVariables::calculate_var_from_lec(const std::string& lec, const int t_first, const int t_last)
{
    ...
    if(res != 0)
    {
        ...
        error_msg += "Cannot compile the LEC expression '" + lec + "'";
        throw std::runtime_error(error_msg);
    }
...
bool KDBVariables::set_sample(const std::string& from, const std::string& to)
{
	...
	return set_sample(period_from, period_to);
}
...
void KDBVariables::merge_from(const std::string& input_file)
{
    ...
    bool success = from_ptr->load(_input_file_);
    if(!success)
        return;
    ...
    KV_merge_del(shared_from_this(), from_ptr, 1);
}
	...
	const std::string& to, const std::string& variables_list)
{
	...
	int res = KV_extrapolate(shared_from_this(), (int) method, &sample, (char*) pattern.c_str());
