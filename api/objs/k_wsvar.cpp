...
int KV_merge(KDBVariables* kdb1, KDBVariables* kdb2, int replace)
{
    ...
    for(const auto& [name, var] : kdb2->k_objs) 
    {
        ...
        if(!found)
            kdb1->set_obj_ptr(name, new Variable());
        ...
        {
            Variable* var_2 = kdb2->get_obj_ptr(name);
            kdb1->set_obj_ptr(name, new Variable(*var_2));
        }
    }
...
void KDBVariables::set_obj_ptr(const std::string& name, Variable* var_ptr)
{
    ...
    KDBTemplate::set_obj_ptr(key, var_ptr);
}
