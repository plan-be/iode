...
class IodeDatabase:
    ...
    def compare(self, filepath: Union[str, Path], only_in_workspace_list_name: str=None, only_in_file_list_name: str=None, equal_objects_list_name: str=None, different_objects_list_name: str=None) -> Dict[str, List[str]]:
        ...
        >>> vars_other_filepath = str("fun_other.var")
    ...
    def _check_key_single_object(self, key: Union[int, str]) -> Union[int, str]:
        if isinstance(key, int):
            ...
            if not key in self.names:
                raise KeyError(f"Key '{key}' not found in the {type(self).__name__} database")
        else:
