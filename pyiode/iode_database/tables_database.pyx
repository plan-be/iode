...
cdef class Tables(CythonIodeDatabase):
    ...
    def _get_object(self, name: str, table: Table) -> Table:
        cdef CTable* c_table
        name = name.strip()
        c_table = self.database_ptr.get_obj_ptr(name.encode())
    ...
    def _set_object(self, name: str, table: Table):
        ...
        cdef CTable* new_table_ptr = table.c_table
        new_table_ptr = self.database_ptr.set_obj_ptr(name.encode(), table_ptr)
        table.c_table = new_table_ptr
        table.ptr_owner = <bint>False
