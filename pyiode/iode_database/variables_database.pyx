...
cdef inline _df_to_ws_pos(df, start_period: str, end_period:str, int* la_pos, int* ws_pos, int* lg):
    '''
    ...
    '''
    IodeCalcSamplePosition(_cstr(start_period), _cstr(end_period), la_pos, ws_pos, lg)
...
cdef class _VariablesDatabase(_AbstractDatabase):
    ...
    def _unfold_key(self, key) -> List[str]:
        ...
        if isinstance(key, tuple):
            ...
            # periods_ represent a unique period or a slice
            if isinstance(periods_, str): 
                # convert to slice and continue
                if ':' in periods_:
                    args = periods_.split(':')
                    if not (2 <= len(args) <= 3):
                        raise ValueError("Variables[name, periods]: 'periods' must follow the Python slice syntax, i.e. "
                                         "start_period':'end_period'[:'step'] where 'step' is optional.")
                    periods_ = slice(*args)
                # periods_ represents a unique period
                else:
                    return [name, periods_]
            
            # convert slice to list of periods
            if isinstance(periods_, slice):
                sample = self.sample
                start_period = sample.start if periods_.start is None else periods_.start
                end_period = sample.end if periods_.stop is None else periods_.stop
                step = periods_.step
                periods_ = self.periods_subset(start_period, end_period)
                if step is not None:
                    periods_ = periods_[::step]
                return [name, periods_]
            ...
            raise TypeError(f"Variables[name, periods]: 'periods' must represent either a period (str), a list of periods "
                            f"or a slice of periods (start_period:end_period). 'periods' is of type {type(periods_).__name__}.")
    ...
    def _get_object(self, key):
        ...
        # return the variable values for the selected periods
            ...
            # periods_ represents a unique period 
            if isinstance(periods_, str):
                return self.database_ptr.get_var(name.encode(), periods_.encode(), self.mode_)
            # periods_ represents a list of periods
            else:
                return [self.database_ptr.get_var(name.encode(), period_.encode(), self.mode_) for period_ in periods_]

    def _check_set_value(self, value, nb_periods_) -> Union[str, float, List[float]]:
        # value is a LEC expression
        if isinstance(value, str):
            return value
        # same float value for all periods to be set
        if isinstance(value, float):
            return [value] * nb_periods_
        # iterable of float
        if isinstance(value, Iterable) and all(isinstance(item, float) for item in value):
            if len(value) == nb_periods_:
                return value
            else:
                raise ValueError(f"Variables[key] = values: Expected 'values' to be a vector of length {nb_periods_} " 
                                 f"but got vector of length {len(value)}.")    
        # wrong type for 'value'
        raise TypeError("Variables[key] = values: Expected a float or a list of float as 'value'. "
                            f"Got 'value' of type {type(value).__name__}") 

    def _set_object(self, key, value):
        cdef string cpp_str_name
        cdef string cpp_str_period
        ...
        # set variable values for a selection of periods
        else:            
            name, periods_ = key
            if isinstance(periods_, str):
                periods_ = [periods_]
            values = self._check_set_value(value, len(periods_))
            cpp_str_name = name.encode()
            for period_, value_ in zip(periods_, values):
                cpp_str_period = period_.encode()
                self.database_ptr.set_var(cpp_str_name, cpp_str_period, <double>value_, self.mode_)
    
    def add(self, name: str, values: Union[str, float, List[float]]):
        cdef string cpp_str_name
        cdef string cpp_str_lec
        ...
        values = self._check_set_value(values, self.nb_periods)
        # values is a LEC expression
        if isinstance(values, str):
            cpp_str_lec = values.encode()
            self.database_ptr.add(cpp_str_name, cpp_str_lec)
        # values is a vector of float
        else:
            cpp_values = [<double>value_ for value_ in values]
            self.database_ptr.add(cpp_str_name, cpp_values)
    ...
    def update(self, name: str, values: Union[str, float, List[float]]):
        ...
        if isinstance(values, str):
            cpp_str_lec = values.encode()
            self.database_ptr.update(cpp_str_name, cpp_str_lec)
        # values is a vector of float
        else:
            cpp_values = [<double>value_ for value_ in values]
            self.database_ptr.update(cpp_str_name, cpp_values)
    ...
    def from_frame(self, df: DataFrame):
        ...
        start_period, end_period = periods_list[0], periods_list[-1]
        self.sample = f"{start_period}:{end_period}"
        ...
        IodeCalcSamplePosition(_cstr(start_period), _cstr(end_period), &df_pos, &ws_pos, &lg)
    ...
    def sample(self, value: Union[str, slice, Tuple[Union[str, Period], Union[str, Period]]]):
        ...
        value: str or slice(str, str) or tuple(str, str)
            New sample as string 'start_period:end_period' or as a slice 'start_period':'end_period' 
            or as a tuple 'start_period', 'end_period'.
        ...
        if isinstance(value, str):
            ...
            raise TypeError("Variables.sample = value: 'value' must be either a string 'start_period:end_period', "
                            "a slice 'start_period':'end_period' or a tuple 'start_period', 'end_period'.\n "
                            f"Got 'value' of type {type(value).__name__}")
