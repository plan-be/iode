...
cdef class Variables(CythonIodeDatabase):
    ...
    def _get_periods_bounds(self) -> Tuple[int, int]:
        if self.database.get_sample().get() is NULL:
            warnings.warn("The sample of the IODE Variables workspace is not defined")
            return 0, 0
        return self.database.get_first_period_position(), self.database.get_last_period_position()
    ...
    def initialize_subset(self, pattern: str, copy: bool, 
        ...
        cdef string c_first_period = <string>b""
        cdef string c_last_period = <string>b""
        if first_period is not None:
            c_first_period = (<Period>first_period).c_period.to_string()
        if last_period is not None:
            c_last_period = (<Period>last_period).c_period.to_string()
        cdef shared_ptr[KDBVariables] subset_db_ptr = self.database.get_subset(
            pattern.encode(), <bint>copy, c_first_period, c_last_period)
    ...
    def _get_whole_sample(self) -> Sample:
        cdef shared_ptr[CSample] sample_ptr = self.database.get_sample()
    ...
    def _maybe_update_subset_sample(self):
        self.database.update_subset_sample()

    cdef int c_get_real_period_position(self, CPeriod c_period):
        return self.database.get_real_period_position(c_period)
    ...
    def _get_variable(self, name: str, key_periods: Union[Period, List[Period]]) -> Union[float, List[float]]:
        ...
        if isinstance(key_periods, Period):
            ...
            cpp_value = self.database.get_var(c_name, t, self.mode_)
    ...
    def _add(self, name: str, values: Union[str, float, np.ndarray, Iterable[float], Variables]):
        ...
        if isinstance(values, str):
            self.database.add(<string>(name.encode()), <string>values.encode())
        # values is a vector of float
        elif isinstance(values, float):
            cpp_values = [values] * self.get_sample().get_nb_periods()
            self.database.add(<string>(name.encode()), cpp_values)
        elif isinstance(values, np.ndarray):
            numpy_data_memview = values
            b_name = name.encode()
            s_name = b_name
            success = self.database.set_var(s_name, &numpy_data_memview[0])
            if not success:
    ...
    def _update_variable(self, name: str, values, key_periods: Union[Period, tuple, list]):
        ...
        if isinstance(key_periods, Period):
            ...
            # values is a LEC expression
                ...
                self.database.update(c_name, <string>values.encode(), t, t) 
            elif isinstance(values, Variables):
        ...
        # update values for a contiguous range of periods
            ...
            # values is a LEC expression
                ...
                self.database.update(c_name, <string>values.encode(), t_first, t_last) 
            ...
            # values is a numpy array AND the mode is VAR_MODE_LEVEL
                    ...
                    var_ptr = self.database.get_var_ptr(c_name, t_first)
                    numpy_data_memview = values
    ...
    def binary_op_scalar(self, other: float, op: BinaryOperation, copy_self: bool) -> Variables:
        ...
        cdef KDBVariables* cpp_self_db = self.database
    ...
    def binary_op_variables(self, cython_other: Variables, op: BinaryOperation, names: List[str], copy_self: bool) -> Variables:
        cdef int i_op = int(op)
        cdef KDBVariables* cpp_self_db = self.database
        cdef KDBVariables* cpp_other_db = cython_other.database
    ...
    def from_numpy(self, data: np.ndarray, vars_names: List[str], new_vars: Set[str], t_first_period: int, t_last_period: int):
        ...
        if self.mode_ == IodeVarMode.VAR_MODE_LEVEL:
            ...
            for i, name in enumerate(vars_names):
                ...
                #       and then convert the bytes object to a C string using the syntax char_obj = bytes_obj.
                b_name = name.encode()
                if not self.database.contains(<string>b_name):
                    raise RuntimeError(f"Variable '{name}' not found in the IODE Variables database")
                var_ptr = self.database.get_var_ptr(<string>b_name, t_first_period)
    ...
    def to_numpy(self) -> np.ndarray:
        ...
        i = 0
        cdef string c_name
        cdef cpp_set[string] cpp_names = self.database.get_names()
        if self.mode_ == IodeVarMode.VAR_MODE_LEVEL:
            # WARNING: do not use enumerate() here as it does not respect the C++ set order
            for c_name in cpp_names:
                var_ptr = self.database.get_var_ptr(c_name, t_first_period)
    ...
    def get_first_period(self) -> Period:
        ...
        return sample.get_start() if sample is not None else None

    def get_last_period(self) -> Period:
        sample: Sample = self.get_sample()
        return sample.get_end() if sample is not None else None

    def get_is_subset_over_periods(self) -> bool:
        return self.database.is_subset_over_periods()

    def get_sample(self) -> Sample:
        cdef shared_ptr[CSample] sample_ptr = self.database.get_visible_sample()
        cdef CSample* c_sample = sample_ptr.get()
        if c_sample is NULL:
            return None
        if c_sample.start_period.year == 0 or c_sample.nb_periods == 0:
    ...
    def periods_subset(self, from_period: str, to_period: str, as_float: bool) -> List[Union[str, float]]:
        ...
        if as_float:
            return self.database.get_list_periods_as_float(s_from_period, s_to_period)
        else:
            return [cpp_period.decode() for cpp_period in 
                    self.database.get_list_periods(s_from_period, s_to_period)]
    ...
    def merge(self, other: Variables, overwrite: bool=True):        
        cdef shared_ptr[KDBVariables] other_database = other.database_ptr
    ...
    def extrapolate(self, method: int, from_period: str, to_period: str, variables_list: str):
        self.database.extrapolate(<VariablesInitialization>method, from_period.encode(), to_period.encode(), 
                                      variables_list.encode())

    def seasonal_adjustment(self, input_file: str, eps_test: float, series: str):
        self.database.seasonal_adjustment(input_file.encode(), series.encode(), eps_test)

    def trend_correction(self, input_file: str, lambda_: float, series: str, log: bool):
        self.database.trend_correction(input_file.encode(), lambda_, series.encode(), <bint>log)
    ...
    def _str_table(self, names: List[str], periods: List[str]) -> Dict[str, List[float]]:
        ...
        for str_period in periods:
            ...
            columns[str_period] = [self.database.get_var(<str>name.encode('utf-8'), <int>t, self.mode_) 
                                   for name in names]
        return columns
